...
class DynamicCRF(nn.Module):
    """Dynamic CRF layer is used to approximate the traditional
       ...
       in the following two aspects:用E1和E2逼近矩阵M
    ...
    def _compute_score(self, emissions, targets, masks=None):
        ...
        '''print("targets")
        print(targets.shape)#(batch_size, seq_len) 
        print(targets)'''
        emission_scores = emissions.gather(2, targets[:, :, None])[:, :, 0]  # B x T 为待训练的参数--发射矩阵 （序列维度seq_len，标签维度vocab_size）?
        transition_scores = (self.E1(targets[:, :-1]) * self.E2(targets[:, 1:])).sum(2) #为待训练的参数--标签转移矩阵 （标签维度,标签维度）?
        '''print("emission_scores")
        print(emission_scores.shape)#(batch_size, seq_len-1)
        print(emission_scores)
        print("transition_scores")
        print(transition_scores.shape)#(batch_size, seq_len) 
        print(transition_scores)'''

        scores = emission_scores
        scores[:, 1:] += transition_scores
        '''print("scores")
        print(scores.shape)#(batch_size, seq_len-1) 为每句的score
        print(scores)'''

        if masks is not None:
            scores = scores * masks.type_as(scores)
        '''print("scores.sum(-1)")
        print(scores.sum(-1))#(batch_size) 为每句的score'''
