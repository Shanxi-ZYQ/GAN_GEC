...
import numpy as np
import re
from utils.google_bert import create_instances_from_document
...
class Vocab(object):
    def __init__(self, filename, min_occur_cnt, specials = None):
        ...
        idx2token = [PAD, UNK, NUM, NOT_CHINESE] + ( specials if specials is not None else [])#specials是另外加的特殊字符
        ...
        #token2idx 把字映射到字典，把字映射成值，这不是bert的word embdding？
...
#判断s是否是中文编码，下方if语句是中文编码范围，如果不是中文就会返回true
